...
for size in "1M" "10M" "100M" "1G" "10G" ; do
  ...
  tera_input=/tera/${SIZE}-terasort-input
  ...
  echo $DATE
  tera_output=/tera/${SIZE}-terasort-output
  echo "Running terasort"
  timeout 60s time bin/hadoop jar $MR_EXAMPLES_JAR terasort \
  -Dmapreduce.map.log.level=INFO \
  -Dmapreduce.reduce.log.level=INFO \
  -Dyarn.app.mapreduce.am.log.level=INFO \
  -Dio.file.buffer.size=131072 \
  ...
  ${tera_input} ${tera_output}
  echo "Running terasort (done)"
  echo $DATE
